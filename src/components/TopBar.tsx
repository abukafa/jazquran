    ...
    <div className="bg-sage-600 text-white px-5 pt-3 pb-2 text-xs flex justify-between items-center z-50 shadow-sm shrink-0">
      ...
      <div className="flex items-center gap-3">
        <button 
          ...
          className={`${state.isOnline ? 'bg-emerald-400' : 'bg-slate-400'} text-slate-950 font-bold px-2 py-0.5 rounded-full flex items-center gap-1 cursor-pointer hover:opacity-90 transition`}
        >
          {state.isOnline && <span className="w-2 h-2 rounded-full bg-white animate-ping"></span>}
        ...
        {state.syncQueue.length > 0 && (
          <div className="bg-amber-400 text-slate-950 px-2 py-0.5 rounded-full font-bold">
            <i className="fa-solid fa-rotate animate-spin"></i> <span>{state.syncQueue.length}</span> Sync
          </div>
        )}
      </div>
    </div>
