  ...
  return (
    <div className="flex-1 overflow-y-auto pb-24 relative p-6 flex flex-col justify-between h-full min-h-[750px] page-transition">
      <div className="text-center pt-6">
        ...
        <p className="text-slate-500 text-sm mt-2">Autentikasi menggunakan akun Google yang terdaftar di lembaga Anda.</p>
      ...
      <div className="my-6 space-y-4">
        <div className="relative flex items-center py-2">
          ...
          <span className="flex-shrink-0 mx-4 text-slate-400 text-xs font-semibold uppercase">Auth Resmi</span>
          <div className="flex-grow border-t border-slate-200"></div>
        </div>
        ...
        <p className="text-center text-xs text-slate-400">
          Sistem otomatis mendeteksi tenant & ruang kelas berdasarkan email Anda.
        </p>
      </div>
    </div>
  );
...
export default function LoginPage() {
  return (
    <>
      <TopBar />
      <Suspense fallback={<div className="p-6 text-center mt-20 text-slate-500">Memuat...</div>}>
        <LoginForm />
      </Suspense>
    </>
  );
